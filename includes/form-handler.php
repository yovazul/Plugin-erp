...
function dcf_process_form() {
    ...
    if (!empty($form_data['company_name'])) {
        ...
        if (!$result['success']) {
            wp_send_json_error(array(
                'message' => get_option('dcf_error_message') . ' (Thirdparty: ' . $result['error'] . ')'
            ));
        }
        
        $thirdparty_id = isset($result['data']) ? $result['data'] : null;
    }
    ...
    if (!$contact_result['success']) {
        wp_send_json_error(array(
            'message' => get_option('dcf_error_message') . ' (Contact: ' . $contact_result['error'] . ')'
        ));
    }
    
    // Opcionalmente crear ticket con el mensaje
    $contact_id = isset($contact_result['data']) ? $contact_result['data'] : null;
    $api->create_ticket($form_data, $contact_id);
